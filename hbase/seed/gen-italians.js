...
const fs = require('fs');
const filename = 'generated-italians.txt'
...
let cities = [ "Milan", "Florence", "Veneza", "Roma", "Torino", "Naples", "Pisa", "Bologne", "Palermo"]
...
writeToFile("create_namespace 'italians'")
writeToFile("create 'italians', 'personal-data', 'app-data', 'professional-data', 'miscelaneous")

for (let i = 1; i < 10001; i++) {
    let person = randPerson(i+100);

    writeToFile(`put 'italians', '${i}', 'personal-data:firstname', '${person.firstname}'`)
    writeToFile(`put 'italians', '${i}', 'personal-data:surname', '${person.surname}'`)
    writeToFile(`put 'italians', '${i}', 'personal-data:age', '${person.age}'`)
    writeToFile(`put 'italians', '${i}', 'personal-data:bloodType', '${person.bloodType}'`)
    writeToFile(`put 'italians', '${i}', 'personal-data:city', '${person.city}'`)

    if (person.father) {
        writeToFile(`put 'italians', '${i}', 'personal-data:father', '${person.father.firstname}'`)
    }

    if (person.mother) {
        writeToFile(`put 'italians', '${i}', 'personal-data:mother', '${person.mother.firstname}'`)
    }

    writeToFile(`put 'italians', '${i}', 'app-data:id_num', '${person.id_num}'`)
    writeToFile(`put 'italians', '${i}', 'app-data:email', '${person.email}'`)
    writeToFile(`put 'italians', '${i}', 'app-data:registerDate', '${person.registerDate}'`)
    writeToFile(`put 'italians', '${i}', 'app-data:ticketNumber', '${person.ticketNumber}'`)
    writeToFile(`put 'italians', '${i}', 'app-data:username', '${person.username}'`)

    writeToFile(`put 'italians', '${i}', 'professional-data:jobs', '${person.jobs}'`)
    writeToFile(`put 'italians', '${i}', 'professional-data:salary', '${person.salary}'`)

    writeToFile(`put 'italians', '${i}', 'miscelaneous:favFruits', '${person.favFruits}'`)
    writeToFile(`put 'italians', '${i}', 'miscelaneous:movies', '${person.movies.map(m => m.title)}'`)

    if (person.cat) {
        writeToFile(`put 'italians', '${i}', 'miscelaneous:cat', '${person.cat.name}'`)
    }
    
    if (person.dog) {
        writeToFile(`put 'italians', '${i}', 'miscelaneous:dog', '${person.dog.name}'`)
    }
};
